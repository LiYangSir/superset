...
pub fn create_or_attach(
    ...
    std::thread::spawn(move || {
        ...
        read_pty_output(&mut reader, &app_clone, &pid);
    });
...
fn flush_batch(batch: &mut Vec<u8>, app: &tauri::AppHandle, event_name: &str) {
...
fn read_pty_output(reader: &mut File, app: &tauri::AppHandle, pane_id: &str) {
            ...
            match rx.recv_timeout(timeout) {
                Ok(chunk) => {
                    ...
                    if batch.len() >= MAX_BATCH_BYTES {
                        flush_batch(&mut batch, &app_clone, &event_name_clone);
...
pub fn clear_scrollback(_input: serde_json::Value) -> Result<serde_json::Value, String> {
    Ok(serde_json::Value::Null)
}
