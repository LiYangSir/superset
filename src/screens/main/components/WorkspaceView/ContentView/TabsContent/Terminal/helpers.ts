...
import { getHotkeyKeys, isAppHotkeyEvent } from "stores/hotkeys";
import { toXtermTheme } from "stores/theme/utils";
...
function getPreferredRenderer(): PreferredRenderer {
	...
	try {
		...
		if (stored === "canvas") {
			// Canvas renderer was removed in xterm.js 6.0; fall back to auto.
			try {
				localStorage.removeItem("terminal-renderer");
			} catch {
