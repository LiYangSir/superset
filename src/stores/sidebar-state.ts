		...
		persist(
			...
			}),
			{
				...
				migrate: (persistedState: unknown, _version: number) => {
					...
					// Expanded Changes mode should not survive restart
					if (state.currentMode === SidebarMode.Changes) {
						state.currentMode = SidebarMode.Tabs;
					}
				...
				partialize: (state) => ({
					...state,
					currentMode: SidebarMode.Tabs,
					lastMode: state.currentMode === SidebarMode.Changes
						? SidebarMode.Tabs
						: state.lastMode,
				}),
			},
