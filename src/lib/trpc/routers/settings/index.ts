...
// TODO: implement app.relaunch with Tauri API — process.relaunch is Electron-only
...
export const createSettingsRouter = () => {
	return router({
		...
		restartApp: publicProcedure.mutation(() => {
			// TODO: implement with Tauri API — app.relaunch() is Electron-only
		...
		getAnthropicApiKey: publicProcedure.query(() => {
			const row = getSettings();
		...
		setAnthropicApiKey: publicProcedure
			...
			.mutation(({ input }) => {
				localDb
					.insert(settings)
					.values({ id: 1, anthropicApiKey: input.key || null })
					.onConflictDoUpdate({
						target: settings.id,
						set: { anthropicApiKey: input.key || null },
					})
					.run();
				return { success: true };
			}),

		getAnthropicBaseUrl: publicProcedure.query(() => {
			const row = getSettings();
			return row.anthropicBaseUrl ?? null;
		}),

		setAnthropicBaseUrl: publicProcedure
			.input(z.object({ url: z.string() }))
			.mutation(({ input }) => {
				localDb
					.insert(settings)
					.values({ id: 1, anthropicBaseUrl: input.url || null })
					.onConflictDoUpdate({
						target: settings.id,
						set: { anthropicBaseUrl: input.url || null },
					})
					.run();
				return { success: true };
			}),

		getAnthropicModel: publicProcedure.query(() => {
			const row = getSettings();
			return row.anthropicModel ?? "deepseek-v4-flash";
		}),

		setAnthropicModel: publicProcedure
			.input(z.object({ model: z.string() }))
			.mutation(({ input }) => {
				localDb
					.insert(settings)
					.values({ id: 1, anthropicModel: input.model || null })
					.onConflictDoUpdate({
						target: settings.id,
						set: { anthropicModel: input.model || null },
					})
					.run();
				return { success: true };
			}),
