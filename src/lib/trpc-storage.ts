...
const pendingSnapshotFlushers: Array<() => void> = [];

/**
 * Flush all pending localStorage snapshots synchronously before page unload.
 * This ensures no state is lost when the WebView is refreshed or the app quits.
 */
export function flushPendingSnapshots(): void {
	for (const flush of pendingSnapshotFlushers) {
		flush();
	}
}
...
function createTrpcStorageAdapter(config: TrpcStorageConfig): StateStorage {
	...
	pendingSnapshotFlushers.push(() => {
		if (pendingSnapshotTimer) {
			clearTimeout(pendingSnapshotTimer);
			pendingSnapshotTimer = null;
		}
		if (pendingSnapshotValue) {
			// We don't have the name here yet; it's captured per-call.
			// Use the lastKnownName captured below.
			if (lastKnownName) {
				try {
					localStorage.setItem(
						getPendingSnapshotKey(lastKnownName),
						pendingSnapshotValue,
					);
					localStorage.setItem(
						getPendingSnapshotUpdatedAtKey(lastKnownName),
						String(Date.now()),
					);
				} catch {
					// Best-effort
	...
	return {
		...
		setItem: async (name: string, value: string): Promise<void> => {
			lastKnownName = name;
			// Guard: if hydration failed (tRPC error on load), don't let the
			// empty default state overwrite persisted data on disk.
			if (!hydrationSucceeded) {
				try {
					const parsed = JSON.parse(value) as { state: unknown };
					const state = parsed.state as Record<string, unknown> | null;
					const isEmpty =
						!state ||
					...
					// If we can't parse it, skip the guard
				}
			}
