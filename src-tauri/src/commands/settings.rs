...
pub fn update_terminal_preset(
    ...
    match preset {
        Some(p) => {
            ...
            if let Some(icon_url) = patch.get("iconUrl") {
                if icon_url.is_null() {
                    p.as_object_mut().map(|obj| obj.remove("iconUrl"));
                } else {
                    p["iconUrl"] = icon_url.clone();
                }
            }
...
pub fn set_preset_icon(
    ...
    let id = input
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| rusqlite::Error::InvalidParameterName("missing id".to_string()))?;
    let icon = input.get("icon").and_then(|v| {
    ...
    match idx {
        Some(i) => {
            match icon {
                Some(data_url) => {
                    presets[i]["iconUrl"] = serde_json::Value::String(data_url.to_string());
                }
                None => {
                    presets[i].as_object_mut().map(|obj| obj.remove("iconUrl"));
                }
            }
            save_terminal_presets(db, &presets, false)?;
            let icon_url = presets[i].get("iconUrl").and_then(|v| v.as_str()).unwrap_or("");
            let result = if icon_url.is_empty() { serde_json::Value::Null } else { json!(icon_url) };
            Ok(json!({ "iconUrl": result }))
        }
        None => Err(rusqlite::Error::InvalidParameterName(format!(
            "Terminal preset {} not found",
            id
