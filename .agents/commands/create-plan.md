...
> - **Desktop app work**: `plans/<timestamp>-<plan-name>.md`
> - **Cross-app/shared work**: `plans/<timestamp>-<plan-name>.md` (root)
...
8. Write your plan to the appropriate location:
   - Desktop app: `plans/<timestamp>-<plan-name>.md`
...
Example questions:
    I reviewed src/lib/workspace-manager.ts to understand workspace patterns.

    Where should this new feature live?
    a) src/ (renderer process)
    b) src-tauri/ (Rust backend)
    c) Other (specify)
...
### Desktop App Rules
For Tauri/desktop work, remember:
- **Renderer process** (`src/`): Browser environment only, NO Node.js imports
- **Rust backend** (`src-tauri/`): Native system access via Tauri commands
...
Anchor the plan with observable outcomes. State what the user can do after implementation, the commands to run, and the outputs they should see. Acceptance should be phrased as behavior a human can verify ("after running `npm run dev`, navigating to localhost:3000/dashboard shows the new widget") rather than internal attributes ("added a DashboardWidget component"). If a change is internal, explain how its impact can still be demonstrated.
...
Validation is not optional. Include instructions to run `npm run typecheck`, `npm run lint`, and `npm test` as appropriate. Describe expected outputs and error messages so a novice can tell success from failure.
...
**Example milestone structure:**
    ...
    ### Milestone 1: Tauri Command Foundation

    This milestone establishes the Tauri command for workspace operations.
    At completion, the Rust backend can receive workspace requests and return
    typed responses, though no UI integration exists yet.

    Scope:
    - Define Tauri command in src-tauri/src/commands/
...
## Skeleton of a Good ExecPlan
    ...
    For this Superset project, always mention:
    - Which areas are affected (renderer in src/, Rust backend in src-tauri/)
    - Any Tauri commands if backend work is involved
    ...
    For Tauri commands, define in src-tauri/src/commands/:

        #[tauri::command]
        pub async fn my_new_command(param1: String) -> Result<MyData, String> {
            // ...
        }
